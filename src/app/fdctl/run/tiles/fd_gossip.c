...
struct fd_gossip_tile_ctx {
  ...
  fd_frag_meta_t * gossip_plugin_out_mcache;
  ulong *          gossip_plugin_out_sync;
  ulong            gossip_plugin_out_depth;
  ulong            gossip_plugin_out_seq;
...
#define COPY_ADDR( _idx_, _srcname_ )                                                    \
  ...
  *(ulong *)dst = i;
  ...
  fd_mcache_publish( ctx->gossip_plugin_out_mcache, ctx->gossip_plugin_out_depth, ctx->gossip_plugin_out_seq, FD_PLUGIN_MSG_GOSSIP_UPDATE, ctx->gossip_plugin_out_chunk, data_sz, 0UL, 0UL, tspub );
...
after_credit( fd_gossip_tile_ctx_t * ctx,
  ...
  *charge_busy = 1;
  ...
  if( ctx->gossip_plugin_out_mem && ( now - ctx->last_plugin_push_time )>PLUGIN_PUBLISH_TIME_NS ) {
    ctx->last_plugin_push_time = now;
    publish_peers_to_plugin( ctx );
  }
...
unprivileged_init( fd_topo_t *      topo,
  ...
  if( FD_LIKELY( tile->replay.plugins_enabled ) ) {
    if( FD_UNLIKELY( tile->out_cnt < 8UL ||
                     strcmp( topo->links[ tile->out_link_id[ PLUGIN_OUT_IDX  ] ].name, "gossip_plugin" ) ) ) {
    ...
    ctx->gossip_plugin_out_mcache      = gossip_plugin_out->mcache;
    ctx->gossip_plugin_out_sync        = fd_mcache_seq_laddr( ctx->gossip_plugin_out_mcache );
    ctx->gossip_plugin_out_depth       = fd_mcache_depth( ctx->gossip_plugin_out_mcache );
    ctx->gossip_plugin_out_seq         = fd_mcache_seq_query( ctx->gossip_plugin_out_sync );
